...
# crontab -l
# 30 03 * * * ~/scp_scripts/full_and_incr_scp_oracle_rman_expdp_or_exp_3.sh
...
# |------------------------------------------------------------------------|
# | DATABASE  : Oracle                                                     |
# | FILE      : full_and_incr_scp_oracle_rman_expdp_or_exp_3.sh            |
...
# |             production.                                                |
# |                                                                        |
# |             The differences both the version 2nd and the 1st one are   |
# |             using a subtle AWK command to compare the content of       |
# |             $TMP_LOCAL_LOG and $TMP_REMOTE_LOG (and the content of     |
# |             $LOCAL_SUBDIR and $REMOTE_SUBDIR) in the former rather     |
# |             than using DIFF command in the latter one.                 |
# |                                                                        |
# |             The differences both the version 3rd and the 2nd one are   |
# |             adding the retention policy (based on the version 2nd) for |
# |             automatically purged backup files which are in the outside |
# |             of retention policy in order to avoid exhausting the disk  |
# |             space.                                                     |
# +------------------------------------------------------------------------+
...
$AWK 'NR==FNR{a[$0]++} NR>FNR&&!a[$0]' $TMP_LOCAL_LOG $TMP_REMOTE_LOG > $TMP_COMPARE_LOG
$AWK 'NR==FNR{a[$0]++} NR>FNR&&!a[$0]' $TMP_LOCAL_LOG $TMP_REMOTE_LOG >> $TMP_COMPARE_LOG_ALL
...
$AWK 'NR==FNR{a[$0]++} NR>FNR&&!a[$0]' $LOCAL_SUBDIR $REMOTE_SUBDIR > $TMP_SUBDIR
$AWK 'NR==FNR{a[$0]++} NR>FNR&&!a[$0]' $LOCAL_SUBDIR $REMOTE_SUBDIR >> $TMP_SUBDIR_ALL
...
if [ $DISK_USED_PERCENT -le 95 ]; then
  for entry in `$CAT $TMP_COMPARE_LOG`
  do
...
# +------------------------------------------------------------------------+
# | Setting the retention policy (only retaining 186 days for exp[dp] and  |
# | 168 days for rman) for automatically purged backup files which are in  |
# | the outside of retention policy in order to avoid exhausting the disk  |
# | space.                                                                 |
# |                                                                        |
# | Ref: https://www.cyberciti.biz/faq/find-command-exclude-ignore-files/  |
# +------------------------------------------------------------------------+

function rm_oracle_rman () {
  LOCAL_SUB_DIR=$1
  
  # Notice: I don't recommend that you directly remove directory notably finding out those directories in the specific path.
  # As you can see from the following demo it has firstly shown the specific path, it is very dangerous!!!
  
  # [oracle@sync_back ~]$ find /home/oracle/test -type d
  # /home/oracle/test  <<== Here it firstly shows the specific path.
  # /home/oracle/test/2021-04-05
  
  # [oracle@sync_back ~]$ find /home/oracle/test -type d \( ! -path "/home/oracle/test" \)
  # /home/oracle/test/2021-04-05
  
  # [oracle@sync_back ~]$ find /home/oracle/test -type d \( ! -path "/home/oracle/test/2021-04-05" \) 
  # /home/oracle/test
  
  ### If you next do the rm operation, the entire directory will be removed. Hence be careful to this dangerous operation.
  
  ### $FIND $LOCAL_SUB_DIR -type d -mtime +168 -exec rm -rf {} \;
  
  $FIND $LOCAL_SUB_DIR -type f \( \( ! -name "*.sh" \) -and \( ! -name "*.sql" \) \) -mtime +168 -exec rm -f {} \;
  $FIND $LOCAL_SUB_DIR -type d -empty -exec rmdir {} \;
}

function rm_oracle_exp[dp] () {
  LOCAL_SUB_DIR=$1
  $FIND $LOCAL_SUB_DIR -type f \( -name "*.dmp" -or -name "*.log" \) -mtime +186 -exec rm -f {} \;
}

# prodb1:

rm_oracle_rman /DATA/prodb1/rman_backup

# prodb2:

rm_oracle_rman /DATA/prodb2/rman_backup
rm_oracle_exp[dp] /DATA/prodb2/expdp

# prodb3:

rm_oracle_rman /DATA/prodb3/backupset
rm_oracle_exp[dp] /DATA/prodb3/expdp

# prodb4:

rm_oracle_rman /DATA/prodb4/rman_backup
rm_oracle_exp[dp] /DATA/prodb4/exp

# prodb5:

rm_oracle_rman /DATA/prodb5/rman_backup
rm_oracle_exp[dp] /DATA/prodb5/exp

# prodb6:

rm_oracle_rman /DATA/prodb6/rman_backup
rm_oracle_exp[dp] /DATA/prodb6/exp

# prodb7:

rm_oracle_rman /DATA/prodb7/rman_backup

# prodb8:

rm_oracle_rman /DATA/prodb8/rman_backup

# prodb9:

rm_oracle_exp[dp] /DATA/prodb9/exp
